...
import queue
import struct  # To handle raw byte data
...
# 🔹 Função para processar dados em bytes e atualizar gráfico
def update_plot(frame):
    ...
    try:
        ...
        # 🔹 Processar dados do ESP32 (em bytes)
        # Aqui, assumimos que cada pacote de dados contém 12 bytes
        if len(data) == 12:  # Cada pacote contém 12 bytes (3 inteiros de 2 bytes cada)
            # Desempacotar os dados (GyX, GyY, GyZ, AcX, AcY, AcZ)
            gy_x_val, gy_y_val, gy_z_val, ac_x_val, ac_y_val, ac_z_val = struct.unpack('>6h', data)
            sample_idx += 1

            # 🔹 Atualizar buffers do gráfico
            x_data.append(sample_idx)
            gy_x.append(gy_x_val)
            gy_y.append(gy_y_val)
            gy_z.append(gy_z_val)

            # 🔹 Armazenar dados para FFT
            raw_gy_x.append(gy_x_val)
            raw_gy_y.append(gy_y_val)
            raw_gy_z.append(gy_z_val)

            # 🔹 Gerar onda sonora baseada no giroscópio
            waveform = process_vibration_data([gy_x_val, gy_y_val, gy_z_val])
